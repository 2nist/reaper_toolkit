...
local script_manager --     end
    
    ImGui.Separator(ctx)
    
    -- Simple Font Selector
    ImGui.Text(ctx, "🔤 Choose Default Font:")
    
    local font_list = require 'font_config'
    local current = _G.get_saved_font and _G.get_saved_font() or "— none —"
    local font_changed = false
    
    if ImGui.BeginCombo(ctx, "##font_combo", current or "— none —") then
      for i, f in ipairs(font_list) do
        local is_selected = (f.name == current)
        if ImGui.Selectable(ctx, f.name, is_selected) then
          if _G.save_font then _G.save_font(f.name) end
          font_changed = true
        end
        if is_selected then
    ...
    if font_changed then
      ImGui.TextColored(ctx, 0xFFFF00FF, "⚠️ Font saved. Restart script to apply.")
    end
    
    ImGui.Separator(ctx)
    
    -- REAPER Status Sectionbe loaded in init
...
-- Utility to get/set the chosen font name
...
-- UI state variablesmanager -- will be loaded in init
local console_logger -- will be loaded in init
...
function M.draw(ctx)
    ...
    if not font_manager then
        ...
        local changed_font, new_font_index = ImGui.Combo(ctx, "Font Family", selected_font_index - 1, table.concat(font_families, "\0") .. "\0")
        if changed_font then
        ...
        local changed_size, new_size_index = ImGui.Combo(ctx, "Font Size", selected_size_index - 1, table.concat(size_strings, "\0") .. "\0")
        if changed_size then
