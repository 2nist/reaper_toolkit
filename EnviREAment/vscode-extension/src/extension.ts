...
export function activate(context: vscode.ExtensionContext) {
    ...
    // Command: Run Tests
    const runTestsCommand = vscode.commands.registerCommand('envireament.runTests', async () => {
        ...
        const testRunnerPath = findEnviREAmentTestRunner(workspaceFolder.uri.fsPath);
        ...
        if (!testRunnerPath) {
            ...
            if (response === 'Install via pip') {
                runCommand('pip install envireament', workspaceFolder.uri.fsPath, outputChannel);
            } else if (response === 'Install via npm') {
                runCommand('npm install envireament', workspaceFolder.uri.fsPath, outputChannel);
            }
            return;
        }
    ...
    // Command: Run Demo
    const runDemoCommand = vscode.commands.registerCommand('envireament.runDemo', async () => {
        const workspaceFolder = vscode.workspace.workspaceFolders?.[0];
        if (!workspaceFolder) {
            vscode.window.showErrorMessage('No workspace folder found');
            return;
    ...
    // Command: Show Status
    const showStatusCommand = vscode.commands.registerCommand('envireament.showStatus', async () => {
        const workspaceFolder = vscode.workspace.workspaceFolders?.[0];
        if (!workspaceFolder) {
            vscode.window.showInformationMessage('EnviREAment: No workspace folder');
            return;
        }

        const hasEnviREAment = findEnviREAmentTestRunner(workspaceFolder.uri.fsPath) !== null;
        const luaFiles = await vscode.workspace.findFiles('**/*.lua', null, 10);
...
• Workspace: ${workspaceFolder.name}`;
    ...
    const openDocsCommand = vscode.commands.registerCommand('envireament.openDocs', () => {
        vscode.env.openExternal(vscode.Uri.parse('https://github.com/your-username/EnviREAment/docs'));
    });
...
function findEnviREAmentTestRunner(workspacePath: string): string | null {
    ...
    const nodeModulesPath = path.join(workspacePath, 'node_modules', 'envireament', 'enhanced_test_runner.lua');
    ...
    try {
        const result = cp.execSync('python3 -c "import envireament; print(envireament.get_virtual_reaper_path())"', 
                                 { encoding: 'utf8', timeout: 5000 });
        const virtualReaperPath = result.trim();
        const testRunnerPath = path.join(path.dirname(virtualReaperPath), 'enhanced_test_runner.lua');
...
function findEnviREAmentDemo(workspacePath: string): string | null {
    ...
    // Look for EnviREAment installation
    const nodeModulesPath = path.join(workspacePath, 'node_modules', 'envireament', 'examples', 'main.lua');
    if (fs.existsSync(nodeModulesPath)) {
        return nodeModulesPath;
    }
    ...
    // Check if Python package is available
    try {
        const result = cp.execSync('python3 -c "import envireament; print(envireament.get_examples_dir())"', 
                                 { encoding: 'utf8', timeout: 5000 });
...
function runCommand(command: string, cwd: string, outputChannel: vscode.OutputChannel) {
    ...
    process.on('close', (code) => {
        if (code === 0) {
            ...
            vscode.window.showInformationMessage('EnviREAment command completed successfully');
        } else {
            outputChannel.appendLine(`\n❌ Command failed with exit code ${code}`);
            vscode.window.showErrorMessage(`EnviREAment command failed with exit code ${code}`);
        }
    });

    process.on('error', (error) => {
        outputChannel.appendLine(`\n❌ Command error: ${error.message}`);
        vscode.window.showErrorMessage(`EnviREAment command error: ${error.message}`);
    });
}
